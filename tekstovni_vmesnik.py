...
def pozeni_igro():
    ...
    def validity_check_move_piece():
        a, b = copy.deepcopy(igra.polje), copy.deepcopy(igra.povezave)
        backups.append((a, b))
        ...
        while True:
            ...
            elif igra.barva_polja(start) != Now_playing[0].lower():
        ...
        print('1. up\n2. down\n3. left\n4. right')
        izbira = input('>>>')
        while izbira not in ['1', '2', '3', '4']:
            izbira = input('>>>')
        slovarcek = {'1': (-1, 0), '2': (1, 0), '3': (0, -1), '4': (0, 1)}
        dovoljen_premik = slovarcek[izbira]
    ...
    def validity_check_new_piece(ploscek = 'f'):
        ...
        while True:
            move = validity_check_input()
            if igra.je_zasedeno(move) or igra.poteza(Now_playing[0].lower() ,move, ploscek):
                print('Invalid move')
            else:
                break
    ...
    def izpis_konca_igre(igra):
        if igra.konec_igre():
            ...
            return '-----{color} wins-----'.format(color = Now_playing)
    ...
    while igra.konec_igre() is False:
        Text = '---{colour} to play---\n1. New piece\n2. Move piece\n3. Board state\n4. Undo\n5. Resign'.format(colour = Now_playing)
        print(Text)
        izbira = input('>>>')
        ...
        if izbira == '1':
            if igra.ploscki[Now_playing][1] > 0:
                print('1. Flat\n2. Wall\n3. Capstone')
                izbira2 = input('>>>')
                while izbira2 not in ['1','2','3']:
                    izbira2 = input('>>>')
                else:
            ...
            if backups:    
                igra.polje, igra.povezave = backups[-1][0], backups[-1][1]
                backups.pop(-1)
                switch_player()
                print(izpis_igre(igra))
